...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(mainAppCurrentLanguage)]) {
        return [self.delegate mainAppCurrentLanguage];
    }else{
        return @"";
    }
}
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(mainAppNetworkSymbol)]) {
        return [self.delegate mainAppNetworkSymbol];
    }else{
        return @"";
    }
}
